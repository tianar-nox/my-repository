...
class Formatter {
  ...
  public function situplay_link_callback($matches) {
		$url = 'http://bbs2.tunaground.net/trace/' . $matches[1] . '/' . $matches[2] . '/' . $matches[3];
		$text = htmlspecialchars($matches[0], ENT_QUOTES, 'UTF-8');
  ...
  function __construct($page="",$options=array()) {
    ...
    $this->baserule=array("/(?<!\<)<(?=[^<>]*>)/",
                     ...
                     "/`(?<!\s)(?!`)([^`']+)(?<!\s)'(?=\s|$)/",
                     "/`(?<!\s)(?U)(.*)(?<!\s)`/",
                     "/\b(situplay)>([0-9]+)>([0-9]+)\b/", // situplay 링크를 찾는 규칙
                     "/^(={4,})$/",
                     "/,,([^,]{1,40}),,/",
                     "/\^([^ \^]+)\^(?=\s|$)/",
                     "/\^\^(?<!\s)(?!\^)(?U)(.+)(?<!\s)\^\^/",
    ...
    $this->baserepl=array("&lt;",
                     ...
                     "<em>\\1</em>",
                     "&#96;\\1'","<code>\\1</code>",
                     array($this, 'situplay_link_callback'), // situplay 링크를 처리할 함수를 지정
                     "<br clear='all' />",
                     "<sub>\\1</sub>",
                     "<sup>\\1</sup>",
                     "<sup>\\1</sup>",
